...
#### Примечание: единицы учета (текущая реализация)

- Ledger хранит **кредитные единицы**. Для транспорта это байты, для услуг — стоимость услуги, для VPN — цена за MB.
- `debt_limit` применяется ко всем типам долга, поэтому единицы **смешаны**.
- On‑chain расчёты идут в **SIBR**, прямой маппинг IOU → SIBR пока не унифицирован.
- В логах возможны `balance mismatch` из‑за несовпадения единиц и асимметрии учёта.
...
Интерфейс к Siberium blockchain с нативным токеном SIBR:

**[KEY DIFFERENCE]** SIBR - это native gas token (как ETH на Ethereum). Нет ERC-20 контракта - используются native transfers через `web3.eth.send_transaction`.
...
ZEOSettlement.sol позволяет:
- **Deposit (payable):** Stake native SIBR
- **Claim:** Забрать settlement payment через подписанный IOU
- **Unstake flow:** request_unstake -> wait 7 days -> withdraw

**Код:** [`economy/chain.py:287-422`](economy/chain.py)
...
**Код:** [`economy/chain.py:287-532`](economy/chain.py)
...
ZEONE использует Siberium blockchain для on-chain settlement:
...
- EVM-compatible (использует Web3.py)
- PoA consensus с geth_poa_middleware
- Низкие gas fees для micropayments
...
**Код:** [`economy/chain.py:68-94`](economy/chain.py) - Network configurations
